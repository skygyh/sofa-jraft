...
public abstract class AbstractRheaKVStoreTest extends RheaKVTestCluster {
    ...
    private void keyNumberTest(RheaKVStore store) {
        ...
        store.bPut(key1, value);
        Assert.assertEquals(1L, store.bSize().longValue());
        final String key2 = "b_key_number_test";
        store.bPut(key2, value);
        Assert.assertEquals(2L, store.bSize().longValue());
    }
    ...
    private void batchCompositeTest(RheaKVStore store) {
        ...
        for (int i = 0; i < 10; i++) {
            ...
            checkRegion(store, key2, 2);
            if (i % 2 == 0) {
                entries3.add(new KVOperation(key1, makeValue("batch_composite_test_value" + i * 100), null,
                    KVOperation.PUT));
                entries3.add(new KVOperation(key2, makeValue("batch_composite_test_value" + i * 100), null,
                    KVOperation.PUT));
            } else {
