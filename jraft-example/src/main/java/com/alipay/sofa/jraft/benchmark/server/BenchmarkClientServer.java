...
public class BenchmarkClientServer {
    private static final Logger LOG      = LoggerFactory.getLogger(BenchmarkClientServer.class);
    ...
    public static void main(final String[] args) {
        ...
        Runtime.getRuntime().addShutdownHook(new Thread(rheaKVStore::shutdown));
        LOG.info("BenchmarkServer start OK, options: {}", opts);

        Endpoint self = opts.getStoreEngineOptions().getServerAddress();
        final String[] peerStrs = StringUtils.split(initialServerList, ',');
        LOG.info("the first server in the initialServerList is {}", peerStrs[0]);
        final String[] leaderId = StringUtils.split(peerStrs[0], ':');
        int port =  Integer.parseInt(leaderId[1]);
        Endpoint leader = new Endpoint(leaderId[0], port);
        LOG.info("self endpoint {}, leader endpoint {}", self, leader);


        //rebalance2(rheaKVStore, initialServerList, regionRouteTableOptionsList);

        rebalance(rheaKVStore, initialServerList, regionRouteTableOptionsList, self, leader);
    ...
    public static void rebalance(final RheaKVStore rheaKVStore, final String initialServerList,
        ...
        for (;;) {
            ...
            try {
                endpoint = pdClient.getLeader(regionId, true, 10000);
                if (endpoint == null) {
                    regions.add(regionId);
                    continue;
                }

                if (endpoint.equals(leader)) {
                    continue;
                }
            ...
            try {
                if (!endpoint.equals(leader)) {
                    pdClient.transferLeader(regionId, JRaftHelper.toPeer(new PeerId(leader, 0)), true);

                    LOG.info("Region {} transfer from owner {} to leader {}", regionId, endpoint, leader);
                    regions.add(regionId);
                }
            } catch (final Exception e) {
                LOG.error("Fail to transfer leader to {}", self);
                regions.add(regionId);
            }
    ...
    // Because we use fake PD, so we need manual rebalance
    public static void rebalance2(final RheaKVStore rheaKVStore, final String initialServerList,
                                 final List<RegionRouteTableOptions> regionRouteTableOptionsList) {
        final PlacementDriverClient pdClient = rheaKVStore.getPlacementDriverClient();
